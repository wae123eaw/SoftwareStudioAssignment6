...
+ 當游標拖曳一角色，若可以放在網路內時(在大園圈內)，則大園會變色。
...
※曲線的實作：
利用curve方法，並且設定控制點為該角色與圓點連線向大園外延伸。
...
但我們一開始遇到時是在讀取時產生的。
一開始是只利用一個ArrayList去讀取JASON檔。讀取資料是在鍵盤紐按下後，開一個新的arrayList，將資料讀進來。這個設計下，在做快速的影集切換時，會誘發問題。
